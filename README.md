...
- ⏯ 진행률 표시 + 취소
- 🚗 Korean 폰트 임베디드 (NanumGothic, SIL OFL)
...
4. **다운로드** 한 번 클릭 → 자동으로:
   - 각 날짜의 사용내역을 hipass에서 fetch
   - 각 거래의 영수증 HTML을 fetch
   - PDF / PNG로 변환 (NanumGothic 한글 임베딩)
   - (옵션) ZIP으로 묶기
...
│  - Job orchestrator (per-date list → per-receipt) │
│  - 직접 fetch hipass.co.kr (host_permissions로 쿠키)│
│  - chrome.downloads + onChanged 큐                  │
└────────────────────┬────────────────────────────────┘
                     │ chrome.runtime.sendMessage
                     ▼
┌─────────────────────────────────────────────────────┐
│  Offscreen Document (hidden DOM)                    │
│  - jsPDF (단일/멀티페이지) + html2canvas (PNG)        │
│  - JSZip                                            │
